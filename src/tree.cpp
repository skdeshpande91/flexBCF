...
tree::tree_cp tree::get_bn(double* x_cont, int* x_cat)
{
  ...
  if(rule.is_aa && !rule.is_cat){
    ...
    if(x_cont != 0){
      tmp_x = 0.0;
      for(rc_it it = rule.rc_weight.begin(); it != rule.rc_weight.end(); ++it) tmp_x += (it->second) * x_cont[it->first];
...
/*
 old version
void tree::get_rg_aa(int &v, double &c_lower, double &c_upper){
  // starting at current node, recurse up the tree checking if the parent split on v-th continuous predictor
  if(p){
    if(!p->rule.is_cat && p->rule.v_aa == v){
      // my parent splits on v
      if(this == p->l){
        ...
        // as we recurse up the tree, we don't want to over-write a stricter region
        // if p->rule.c is somehow smaller than p->rule.c then we will over-write
        if(p->rule.c < c_upper) c_upper = p->rule.c;
      } else if(this == p->r){
        // my parent splits on v, so any observation in this should be greater than or equal to p->rule.c
        // this establishes a lower bound on valid cutpoints for v in this:
        if(p->rule.c > c_lower) c_lower = p->rule.c;
      } else Rcpp::stop("[get_rg_aa]: this was not equal to either left or right child of parent!");
    } // closes if checking that (i) p splits on a continuous variable & (ii) p splits on v
