...
/*
 everthing below was used to test the internal workins of flexBCF and allowed us to verify that
 we screwed up the indexing
// convention: all control subjects listed first
// [[Rcpp::export()]]
Rcpp::List flexBART_mu(Rcpp::NumericVector Y_train,
                   ...
                   bool verbose, int print_every)
{
  ...
  std::vector<tree> t_mu_vec(M);

  std::vector<suff_stat> ss_train_mu_vec(M);
  ...
  arma::mat mu_fit_samples = arma::zeros<arma::mat>(nd,n_train);
  ...
  for(int iter = 0; iter < total_draws; iter++){
    ...
    if( (iter >= burn) && ( (iter - burn)%thin == 0)){
      ...
      Rcpp::CharacterVector mu_tree_string_vec(M);
      for(int m = 0; m < M; m++){
        mu_tree_string_vec[m] = write_tree(t_mu_vec[m], tree_pi_mu, set_str);
      }
      ...
      for(int m = 0; m < M; m++){
        for(suff_stat_it ss_it = ss_train_mu_vec[m].begin(); ss_it != ss_train_mu_vec[m].end(); ++ss_it){
          tmp_mu = t_mu_vec[m].get_ptr(ss_it->first)->get_mu();
          for(int_it it = ss_it->second.begin(); it != ss_it->second.end(); ++it){
            mu_fit_samples(sample_index,*it) += tmp_mu;
          }
        }
      }
...
// only fits based on tau.

// [[Rcpp::export()]]
Rcpp::List flexBART_tau(Rcpp::NumericVector Y_train,
                   Rcpp::IntegerVector treated,
                   Rcpp::NumericMatrix tX_cont_mu_train,
                   Rcpp::IntegerMatrix tX_cat_mu_train,
                   ...
                   bool verbose, int print_every)
{
  ...
  if(verbose){
    ...
    Rcpp::Rcout << " p_cont_tau = " << p_cont_mu << "  p_cat_tau = " << p_cat_tau << std::endl;
  }
  ...
  if(p_cat_tau > 0){
    ...
    if(adj_support_list_tau.isNotNull()){
      ...
      parse_cat_adj(adj_support_tau, p_cat_tau, tmp_adj_support, graph_split_mu);
    }
  }
  ...
  tree_pi_tau.var_count = &var_count_mu;
  ...
  tree_pi_tau.unif_cuts = unif_cuts_mu.begin(); // do we use uniform cutpoints?
  tree_pi_tau.cutpoints = &cutpoints_mu;
  ...
  // initialize the trees
  std::vector<tree> t_mu_vec(M);
  std::vector<tree> t_tau_vec(M);

  std::vector<suff_stat> ss_train_mu_vec(M);
  std::vector<suff_stat> ss_train_tau_vec(M);
  ...
  for(int iter = 0; iter < total_draws; iter++){
    ...
    if( (iter >= burn) && ( (iter - burn)%thin == 0)){
      ...
      Rcpp::CharacterVector mu_tree_string_vec(M);
      Rcpp::CharacterVector tau_tree_string_vec(M);
      for(int m = 0; m < M; m++){
        mu_tree_string_vec[m] = write_tree(t_mu_vec[m], tree_pi_mu, set_str);
        tau_tree_string_vec[m] = write_tree(t_tau_vec[m], tree_pi_tau, set_str);
      }
      mu_tree_draws[sample_index] = mu_tree_string_vec;
